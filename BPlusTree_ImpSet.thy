...
lemma split_half_rule[sep_heap_rules]: "<
  ...
  * list_assn R ts tsi> 
  ...
  <\<lambda>i. 
      is_pfa c tsi a
    * list_assn R ts tsi
    * \<up>(i = (length ts + 1) div 2 \<and>  split_relation ts (BPlusTree_Set.split_half ts) i)>"
...
next
  ...
  proof (cases rs)
    ...
    show ?thesis
      ...
      subgoal using "2.prems"  sorted_inorder_separators[of ts t] sorted_wrt_append by blast
      apply(sep_auto)
      subgoal
      ...
      subgoal using "2.prems"(2) sorted_leaves_induct_last by blast
      using 2(3) apply(sep_auto heap: "2.IH"(1)[of ls "[]"] simp add: sorted_wrt_append)
      done
  next
...
(*TODO ai is not necessary not in the heap area of li *)
  "btupi_assn k (abs_split.Up\<^sub>i l a r) (Up\<^sub>i li ai ri) =
   bplustree_assn k l li * \<up>(a = ai) * bplustree_assn k r ri" |
  "btupi_assn _ _ _ = false"



definition node\<^sub>i :: "nat \<Rightarrow> ('a btnode ref option \<times> 'a) pfarray \<Rightarrow> 'a btnode ref \<Rightarrow> 'a btupi Heap" where
...
definition Lnode\<^sub>i :: "nat \<Rightarrow> 'a pfarray  \<Rightarrow> 'a btupi Heap" where
  "Lnode\<^sub>i k a \<equiv> do {
    n \<leftarrow> pfa_length a;
    if n \<le> 2*k then do {
      a' \<leftarrow> pfa_shrink_cap (2*k) a;
      l \<leftarrow> ref (Btleaf a');
      return (T\<^sub>i l)
    }
    else do {
...
definition insert :: "nat \<Rightarrow> ('a::{heap,default,linorder,order_top}) \<Rightarrow> 'a btnode ref \<Rightarrow> 'a btnode ref Heap" where
...
lemma rebalance_middle_tree_update_rule:
  assumes "height tt = height sub"
    ...
    and "i = length ls"
  shows "<is_pfa (2 * k) (zs1 @ (x', sep) # zs2) a * bplustree_assn k sub x' *
     blist_assn k ls zs1 *
     id_assn sep sep *
     blist_assn k rs zs2 *
  ...
  next
    ...
    apply sep_auto 
      ...
      subgoal for lsi subi rsi
        ...
        using "2.prems" apply auto[]
        apply (metis "2.prems"(2) sorted_inorder_induct_subplustree)
        using "2.prems" apply auto[]
...
(* the correct sub branch *)
        subgoal for subnode lsi subi rsi
          apply(subgoal_tac "subi = Some subnode")
          apply (simp del: bplustree_assn.simps)
          supply R = split_max_rule[of "Node subts subt" k "Some subnode"]
          thm R
          apply(sep_auto heap add: R simp del: bplustree_assn.simps)
