...
\begin{abstract}
    ...
    programming languages OCaml, SML and Scala. Considering development time and lines of code, our approach
    compares well to other approaches at mechanized verifications of the \btree\ data
    structure, while being unique in supporting range queries and node internal binary search.
\end{abstract}
...
we define a functional, abstract specification of each operation
...
is reduced to a proof of the refinement relation between the output of the
functional and the imperative implementation.
This process is repeated for each operation throughout
\autoref{sec:set} and \autoref{sec:range}
...
interaction, such as range queries and efficient binary search.
...
Data structure properties such as circle-freeness are then proven by shape analysis in TVLA.
...
namely list interfaces and dynamic arrays.
...
of an imperative implementation of the \btree\ data structure.
...
We also provide the conversion into an iterator over the
leafs based on a flattening iterator we introduced for
iterating over the elements in a list of arrays.
On top, range queries with a lower bound are defined and verified.
...
we concretely export to OCaml, SML and Scala.
...
to another element in the heap, \textbf{ref option} is a pointer that is potentially null.
...
In general, we call these functions \textit{assertions}.
They are functions that take a heap and returns
a statement about the state of the heap.
For this purpose they usually combine other assertions,
the most important here being the star operator
that expresses disjunctness of the parts of the heap
that a statement is being made about.
Later, they are used in the context of Hoare Triples
and express pre- and postconditions of operations.
More details can be found in the work of Lammich. \cite{DBLP:journals/afp/LammichM12}

In this case, the function additionally takes an algebraic tree (\textbf{bplustree})
and an imperative tree in the form of a pointer (\textbf{btnode ref}),
as well as a first and last leaf.
...
and we need to prove that correct linking is not violated when querying or inserting elements.
...
but eventually a binary search like node internal decision process can be verified.
...
specification of the split functionality as well.
...
\footnotetext[4]{
    ...
    the time spent on them can not be accounted for 1:1.
}
