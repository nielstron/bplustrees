...
theorem isin_set_inorder:
  "sorted_less (leaves t)" ; "aligned l t u" $\Longrightarrow$ "isin t x = (x $\in$ set (leaves t))"
\end{lstlisting}
...
lemma isin_rule:
   "k > 0" ; "root_order k t" ; "sorted_less (inorder t)" ; "sorted_less (leaves t)" $\Longrightarrow$
...
lemma insert_rule:
    "k > 0" ; "sorted_less (inorder t)" ; "sorted_less (leaves t)" ; "root_order k t" $\Longrightarrow$
    "<bplustree_assn k t ti r z>
    imp_insert k x ti
...
in the flattening iterator and obtain an iterator over leaf values \emph{leaf\_values\_iter}.
