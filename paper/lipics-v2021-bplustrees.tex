...
lemma  "k > 0 \<Longrightarrow> root_order k t \<Longrightarrow> sorted_less (inorder t) \<Longrightarrow> sorted_less (leaves t) \<Longrightarrow>
   <bplustree_assn k t ti r z>
     isin ti x
   <$\lambda$y. bplustree_assn k t ti r z * $\uparrow$(abs_split_set.isin t x = y)>\<^sub>t"
\end{lstlisting}
...
fun bplustree_assn_leafs :: "nat $\Rightarrow$ ('a::heap) bplustree $\Rightarrow$ 'a btnode ref $\Rightarrow$ 'a btnode ref option $\Rightarrow$ 'a btnode ref option $\Rightarrow$ 'a btnode ref list $\Rightarrow$ assn" where
   ...
   ($\exists_A$ tsi ti tsi' tsi'' rs split.
      ...
      * $\uparrow$(tsi'' = zip (zip (map fst tsi') (zip (butlast (r#rs)) (zip (butlast (rs@[z])) (butlast split)))) (map snd tsi'))
      * list_assn (($\lambda$ t (ti,r',z',lptrs). bplustree_assn_leafs k t (the ti) r' z' lptrs) $\times_a$ id_assn) ts tsi''
      )"
...
lemma first_leaf_rule[sep_heap_rules]:
  ...
  first_leaf ti
  <$\lambda$u. bplustree_assn k t ti r z * $\uparrow$(u = r)>\<^sub>t"
\end{lstlisting}
