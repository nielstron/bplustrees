...
as the abstract leafs are defined such that they store the precise heap pointer to the refining node.
...
to realize that the result is a pointer to a linked-list
view on the \emph{leaf\_nodes}.
The original tree root remains to refine the \emph{trunk}.
In a next step, we show that the linked list can be used
to define an imperative iterator fulfilling the list iterator interface from Lammich. \cite{DBLP:conf/itp/Lammich19}
In order not to loose information about the complete tree,
such that the tree assertion can be re-obtained at the end of the iteration,
the iterator assertion is constructed such that it contains
the assertion on the \emph{trunk}.
...
The outer iterator is the linked list of leaf nodes,
the inner iterator an iterator on the partially filled arrays contained within.
The resulting flattening iterator iterates exactly over \emph{leaves t}.
...
is obtaining a lower bound on some key
or to obtain  all elements within a certain range. \cite{DBLP:journals/ftdb/Graefe11}
We combine these operations with the notion of an \emph{lrange},
the range of elements in the tree, bounded from below by $x$.
An iterator over the \emph{lrange} can be obtained in logarithmic time,
by making use of the tree structure.
The operation is similar to the point query operation,
only that it returns a pointer that is interpreted as iterator.
...
where it is sufficient to return the iterator to the found element.
...
where the last leaf is explicitely a null pointer.\footnote{
    ...
    If the last leaf is a valid leaf node (not a null pointer), and
...
We first formulate this alternative approach on the abstract layer
