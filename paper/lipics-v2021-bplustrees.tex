...
to state that the complete tree does not change through the process.
...
We cannot express that the fringe of the inner tree refines the same abstract leafs
...
We describe the inner nodes with the assertion \emph{inner\_nodes\_assn},
...
In order to express the soundness of the whole tree,
...
The original tree root \emph{t} remains to refine \emph{trunk t}.
We show that the linked list can be used
to define an imperative iterator,
fulfilling the list iterator interface defined by Lammich \cite{DBLP:conf/itp/Lammich19}.
% TODO include this definition
After the iteration we want to express
that the whole tree did not change.
In order to be able to state this,
we need both the leaf nodes assertion and the trunk assertion on \emph{t}.
We therefore define the iterator such that it contains
both assertions.
It also existentially quantifies the fringe,
hiding away the fact that it was extracted in the first place from the client.
% TODO include iterator def

The iterator we have defined now
iterates over the leaf nodes of the \btree.
To really obtain an iterator that returns every single element of the
leaf nodes we introduce a flattening iterator.
...
In this case, the outer iterator is the linked list of leaf nodes,
the inner iterator an iterator on the partially filled arrays in each node.
...
\begin{lstlisting}[mathescape=true, language=Isabelle,label=lst:leaf-iter-init-rule]
