...
All of the four mentioned invariants are proven to be maintained by the abstract set operations.
...
fun bplustree_assn :: nat $\Rightarrow$ 'a bplustree $\Rightarrow$ 'a btnode ref $\Rightarrow$ 'a btnode ref $\Rightarrow$ 'a btnode ref
  ...
  $\exists_A$ xsi fwd.  a $\mapsto_r$ Btleaf xsi fwd * is_pfa (2*k) xs xsi * $\uparrow$(fwd = z) * $\uparrow$(r = Some a)
...
The structural invariant of correct linking is ensured
...
indicating the correct follow up subtree for recursive calls.
We call this function \emph{split}, and define it only by its behavior.
The specification for \emph{split} is given in \cref{fig:split-def}.
...
pointers along the fringe of the tree until we reach the final leaf marked by a null forwarding pointer.
...
In a second attempt make the sharing explicit.
...
It also contains the fringe.
Note how all notion of the explicitly shared leaf pointers
has disappeared on this level, the client perspective,
as their existence is hidden within the definition of the tree iterator.
...
The \textit{has\_next} function in the iterator returns whether there are any remaining elements.
We compare the current leaf with the last leaf of the tree.
If the last leaf is a valid leaf node, and the linked list supposedly empty,
we need to show that the linked leaf list is not cyclic.
...
and we obtain an iterator with the existing definition.
...
of the ubiquitous \btree\ data structure, feauturing range queries and binary search.
